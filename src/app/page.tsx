...
export default function Home() {
  const [holders, setHolders] = useState<Array<{ wallet: string; balance: number }>>([]);
  const [searchedAddress, setSearchedAddress] = useState<string | null>(null);

  const handleSearch = (address: string) => {
    const addressExists = dummyHolders.some(holder => holder.wallet === address);
    if (addressExists) {
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <Skeleton 
        holders={dummyHolders.filter(Boolean)}
